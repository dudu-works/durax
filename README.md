...
마우스를 올리면 뛰지 않고 정면으로 눈을 맞춘 채 `butterfly`, `catch`의 /æ/처럼 입꼬리를 양옆으로 당겨 크게 벌립니다. hover 발성이 끝나면 와와 동작으로 이어지지 않고, 짧은 혀 끝만 내민 채 가만히 있습니다.
...
- 마우스 hover 시 정면을 보며 양옆으로 넓게 벌리는 wide /æ/ 동작
- hover 뒤 와와 발성 없이 짧은 혀 끝을 내민 채 정지하는 동작
- 작은 앞니와 짧게 내민 혀 동작
