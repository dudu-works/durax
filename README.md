...
모든 동작은 하나의 canonical Durax 몸 크기와 발 기준선에서 시작합니다. idle은 화면 중심에서 살짝 비껴난 3/4 정면을 보다가 기존의 왼쪽 시선을 거쳐 다시 3/4 정면으로 돌아옵니다. 마우스를 올리면 같은 기본 몸에서 긴장된 눈매와 볼, 입꼬리로 `아와왁!!` 합니다. hover 뒤에는 자연스러운 짧고 넓은 비율로 쪼그린 채 혀 길이만 왕복합니다.
...
- canonical 표준 면적 18,200px과 공통 발 기준선에서 모든 action 분기
- 일반 action 행 평균 면적 편차를 기존 71%에서 3.2%로 축소
- idle 안에서 `3/4 정면 - 왼쪽 - 왼쪽 깜빡임 - 3/4 정면 복귀` 방향 루프
- idle에서 몸과 발은 고정하고 머리, 눈, 미세한 숨만 변화
